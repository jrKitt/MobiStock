...
export async function GET(req: NextRequest) {
    try {
        ...
        const countResult = await query<{ total: number }[]>('SELECT COUNT(*) as total FROM PURCHASE_ORDER')
        const total = countResult[0].total
        const totalPages = Math.ceil(total / limit)

        const rows = (await query('SELECT * FROM PURCHASE_ORDER ORDER BY po_id DESC LIMIT ? OFFSET ?', [limit, offset])) as PurchaseOrder[]
