...
import { RepairOrder, Customer, ProductItem, SparePart, RepairOrderPart } from '@/types/api'
import { PrintIcon, EditIcon, DeleteIcon, CloseIcon } from '@/lib/icons'

export default function RepairOrdersPage() {
    const { showToast } = useToast()
    const [repairs, setRepairs] = useState<(RepairOrder & { parts?: RepairOrderPart[] })[]>([])
    ...
    const [selectedPartsRepair, setSelectedPartsRepair] = useState<RepairOrder | null>(null)
    ...
    const fetchData = async () => {
        try {
            setLoading(true)
            const [repairsRes, customersRes, itemsRes, partsRes] = await Promise.all([
            ...
            if (!repairsRes.ok || !customersRes.ok || !itemsRes.ok || !partsRes.ok) {
                throw new Error('ไม่สามารถดึงข้อมูลได้')
            }

            const [repairsData, customersData, itemsData, partsData] = await Promise.all([
                repairsRes.json(),
                customersRes.json(),
                itemsRes.json(),
                partsRes.json(),
    ...
    const fetchRepairParts = async (repairId: number) => {
        try {
            const response = await fetch(`/api/repair-order-parts?repair_id=${repairId}`)
            if (response.ok) {
    ...
    const handleEdit = (repair: RepairOrder) => {
        ...
        setFormData({
            ...
            repair_date_received: new Date(repair.repair_date_received).toISOString().split('T')[0],
            repair_date_completed: repair.repair_date_completed ? new Date(repair.repair_date_completed).toISOString().split('T')[0] : '',
    ...
    const handleDelete = async (id: number) => {
        ...
        try {
            const response = await fetch(`/api/repair-orders/${id}`, { method: 'DELETE' })
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault()
        try {
            const url = selectedRepair ? `/api/repair-orders/${selectedRepair.repair_id}` : '/api/repair-orders'
            ...
            showToast(`คำขอซ่อม${selectedRepair ? 'อัปเดต' : 'สร้าง'}สำเร็จ`, 'success')
    ...
    return (
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-2xl font-bold text-slate-900 tracking-tight">คำขอซ่อม</h1>
                    <p className="text-sm text-slate-500 mt-1">จัดการคำขอซ่อมสินค้า และเพิ่มอะไหล่</p>
                </div>
                <button
                    onClick={() => {
                        setSelectedRepair(null)
                    ...
                    <table className="w-full text-left">
                        <thead>
                            <tr className="border-b border-slate-100 bg-slate-50/50">
                                <th className="px-6 py-4 text-xs font-bold tracking-wider text-slate-500 uppercase">วันที่รับ</th>
                                <th className="px-6 py-4 text-xs font-bold tracking-wider text-slate-500 uppercase">ลูกค้า</th>
                                <th className="px-6 py-4 text-xs font-bold tracking-wider text-slate-500 uppercase">ปัญหา</th>
                        ...
                        <tbody className="divide-y divide-slate-100">
                            {repairs.map((repair) => (
                                <tr key={repair.repair_id} className="hover:bg-slate-50/50 transition-colors">
                                    <td className="px-6 py-4 text-sm font-bold text-slate-900">
                                        {new Date(repair.repair_date_received).toLocaleDateString('th-TH')}
                                    </td>
                                    <td className="px-6 py-4 text-sm text-slate-600">
                                        {(() => {
                                            const customer = customers.find(c => c.customer_id === repair.customer_id)
                                            return customer
                                                ? `${customer.customer_fname} ${customer.customer_lname}`
                                                : 'ไม่ระบุ'
                                        })()}
                                    </td>
                                    <td className="px-6 py-4 text-sm text-slate-600">{repair.repair_problem_desc.substring(0, 30)}...</td>
                                    <td className="px-6 py-4">
                                        <div className={`text-xs font-bold uppercase ${getStatusColor(repair.repair_status)}`}>
                                    ...
                                    <td className="px-6 py-4 text-right">
                                        <div className="flex justify-end gap-2">
                                            <button
                                                onClick={() => handleOpenParts(repair)}
                                            ...
                                            <button
                                                onClick={() => repair.repair_id && handleDelete(repair.repair_id)}
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/20 backdrop-blur-sm">
                    <div className="w-full max-w-2xl rounded-xl bg-white p-8 shadow-2xl ring-1 ring-slate-200 max-h-[90vh] overflow-y-auto">
                        <h2 className="text-xl font-bold text-slate-900 mb-6">
                            {selectedRepair ? 'แก้ไขคำขอซ่อม' : 'สร้างคำขอซ่อมใหม่'}
                        </h2>
                        <form onSubmit={handleSubmit} className="grid grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    onChange={(e) => setFormData({ ...formData, repair_date_received: e.target.value })}
                                    className="w-full rounded-md border border-slate-200 px-4 py-2 text-sm focus:border-blue-600 focus:outline-none focus:ring-1 focus:ring-blue-600"
                                />
                            </div>
                            <div>
                                ...
                                <input
                                    ...
                                    onChange={(e) => setFormData({ ...formData, repair_date_completed: e.target.value })}
                                    className="w-full rounded-md border border-slate-200 px-4 py-2 text-sm focus:border-blue-600 focus:outline-none focus:ring-1 focus:ring-blue-600"
                                />
                            </div>
                            <div className="col-span-2">
                                ...
                                <textarea
                                    ...
                                    onChange={(e) => setFormData({ ...formData, repair_problem_desc: e.target.value })}
                                    className="w-full rounded-md border border-slate-200 px-4 py-2 text-sm focus:border-blue-600 focus:outline-none focus:ring-1 focus:ring-blue-600"
                                    rows={3}
                            ...
                            <div className="col-span-2">
                                ...
                                <textarea
                                    ...
                                    onChange={(e) => setFormData({ ...formData, repair_technician_note: e.target.value })}
                                    className="w-full rounded-md border border-slate-200 px-4 py-2 text-sm focus:border-blue-600 focus:outline-none focus:ring-1 focus:ring-blue-600"
                                    rows={2}
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    onChange={(e) => setFormData({ ...formData, repair_labor_cost: parseFloat(e.target.value) })}
                                    className="w-full rounded-md border border-slate-200 px-4 py-2 text-sm focus:border-blue-600 focus:outline-none focus:ring-1 focus:ring-blue-600"
                                    min="0"
                                    step="0.01"
                                />
                    ...
                    <div className="w-full max-w-2xl rounded-xl bg-white p-8 shadow-2xl ring-1 ring-slate-200 max-h-[90vh] overflow-y-auto">
                        <div className="flex items-center justify-between mb-6">
                            <h2 className="text-xl font-bold text-slate-900">
                                จัดการอะไหล่ - {customers.find(c => c.customer_id === selectedPartsRepair.customer_id)?.customer_fname}
                            </h2>
                            <button
                        ...
                        {/* Add Part Form */}
                        <form onSubmit={handleAddPart} className="mb-6 p-4 bg-blue-50 rounded-lg border border-blue-200">
                            <h3 className="text-sm font-bold text-slate-900 mb-4">เพิ่มอะไหล่</h3>
                            <div className="grid grid-cols-3 gap-4">
                                <div>
                                    ...
                                    <select
                                        ...
                                        onChange={(e) => setPartFormData({ ...partFormData, part_id: parseInt(e.target.value) })}
                                        className="w-full rounded-md border border-slate-200 px-4 py-2 text-sm focus:border-blue-600 focus:outline-none focus:ring-1 focus:ring-blue-600"
                                    >
                                ...
                                <div>
                                    ...
                                    <input
                                        ...
                                        required
                                        min="1"
                                        value={partFormData.repair_part_quantity}
                                        onChange={(e) => setPartFormData({ ...partFormData, repair_part_quantity: parseInt(e.target.value) })}
                                        className="w-full rounded-md border border-slate-200 px-4 py-2 text-sm focus:border-blue-600 focus:outline-none focus:ring-1 focus:ring-blue-600"
                                    />
                                </div>
                                <div>
                                    ...
                                    <input
                                        type="number"
                                        required
                                        value={partFormData.repair_part_unit_price}
                                        onChange={(e) => setPartFormData({ ...partFormData, repair_part_unit_price: parseFloat(e.target.value) })}
                                        className="w-full rounded-md border border-slate-200 px-4 py-2 text-sm focus:border-blue-600 focus:outline-none focus:ring-1 focus:ring-blue-600"
                                        min="0"
                                        step="0.01"
                                    />
                        ...
                        {/* Parts List */}
                        <div>
                            <h3 className="text-sm font-bold text-slate-900 mb-4">อะไหล่ที่ใช้</h3>
                            {repairParts.length > 0 ? (
                                <div className="overflow-hidden rounded-lg border border-slate-200">
                                    <table className="w-full text-left text-sm">
                                        <thead>
                                            <tr className="border-b border-slate-100 bg-slate-50/50">
                                                <th className="px-4 py-3 text-xs font-bold tracking-wider text-slate-500 uppercase">อะไหล่</th>
                                                <th className="px-4 py-3 text-xs font-bold tracking-wider text-slate-500 uppercase">จำนวน</th>
                                                <th className="px-4 py-3 text-xs font-bold tracking-wider text-slate-500 uppercase">ราคาต่อหน่วย</th>
                                                <th className="px-4 py-3 text-xs font-bold tracking-wider text-slate-500 uppercase">รวม</th>
                                                <th className="px-4 py-3 text-right text-xs font-bold tracking-wider text-slate-500 uppercase">ลบ</th>
                                            </tr>
                                        </thead>
                                        <tbody className="divide-y divide-slate-100">
                                            {repairParts.map((part) => (
                                                <tr key={`${part.repair_id}-${part.part_id}`} className="hover:bg-slate-50/50">
                                                    <td className="px-4 py-3 text-sm text-slate-600">
                                                        {spareParts.find(p => p.part_id === part.part_id)?.part_name || 'ไม่ระบุ'}
                                                    </td>
                                                    <td className="px-4 py-3 text-sm text-slate-600">{part.repair_part_quantity}</td>
                                                    <td className="px-4 py-3 text-sm text-slate-600">
                                                        ฿{part.repair_part_unit_price.toLocaleString('th-TH')}
                                                    </td>
                                                    <td className="px-4 py-3 text-sm font-bold text-slate-900">
                                                        ฿{(part.repair_part_quantity * part.repair_part_unit_price).toLocaleString('th-TH')}
                                                    </td>
                                        ...
                                        <tfoot>
                                            <tr className="border-t-2 border-slate-200 bg-slate-50/50 font-bold">
                                                <td colSpan={3} className="px-4 py-3 text-right">รวมอะไหล่:</td>
                                                <td colSpan={2} className="px-4 py-3 text-slate-900">
                                                    ฿{repairParts.reduce((sum, p) => sum + (p.repair_part_quantity * p.repair_part_unit_price), 0).toLocaleString('th-TH')}
                                                </td>
                                            </tr>
                                        </tfoot>
                                    </table>
                                ...
                                <div className="text-center py-8 border border-dashed border-slate-200 rounded-lg">
                                    <p className="text-slate-500 text-sm">ยังไม่มีอะไหล่ที่เพิ่ม</p>
                                </div>
                            )}
                        </div>
                    ...
                    <div id="printArea" ref={printRef} className="fixed inset-0 z-50 overflow-auto bg-white p-8 print:p-0">
                        <div className="max-w-4xl mx-auto print:max-w-none">
                            <div className="flex justify-between items-center mb-8 print:hidden">
                                <h1 className="text-3xl font-bold text-slate-900">ใบซ่อม</h1>
                                <button
                            ...
                            <div className="mb-8 pb-8 border-b-2 border-slate-200">
                                ...
                                <p className="text-sm text-slate-600">ระบบจัดเก็บสินค้า - ใบซ่อม</p>
                            </div>
                            ...
                            <div className="grid grid-cols-2 gap-8 mb-8">
                                <div>
                                    ...
                                    <div className="space-y-1">
                                        <p className="text-sm font-bold text-slate-900">
                                            {(() => {
                                                const c = customers.find(cust => cust.customer_id === selectedRepair.customer_id)
                                ...
                                <div className="text-right space-y-2">
                                    <div>
                                        <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">วันที่รับ</p>
                                        <p className="text-sm text-slate-900">{new Date(selectedRepair.repair_date_received).toLocaleDateString('th-TH')}</p>
                                    </div>
                                    <div>
                                        <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">สถานะ</p>
                                        <p className={`text-sm font-bold ${getStatusColor(selectedRepair.repair_status)}`}>
                                            {getStatusLabel(selectedRepair.repair_status)}
                                        </p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="mb-8 p-4 bg-slate-50 rounded-lg">
                                <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-2">ปัญหา</p>
                                <p className="text-sm text-slate-900">{selectedRepair.repair_problem_desc}</p>
                            </div>

                            <div className="mb-8 p-4 bg-slate-50 rounded-lg">
                                <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-2">หมายเหตุช่างซ่อม</p>
                                <p className="text-sm text-slate-900">{selectedRepair.repair_technician_note || '-'}</p>
                            </div>

                            <div className="grid grid-cols-3 gap-4 mb-8 border-t-2 border-b-2 border-slate-200 py-4">
                                <div>
                                    <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">ค่าแรง</p>
                                    <p className="text-lg font-bold text-slate-900">฿{selectedRepair.repair_labor_cost?.toLocaleString('th-TH') || '0'}</p>
                                </div>
                                <div></div>
                                <div className="text-right">
                                    <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">รวมทั้งสิ้น</p>
                                    <p className="text-lg font-bold text-slate-900">฿{(selectedRepair.repair_labor_cost || 0).toLocaleString('th-TH')}</p>
                                </div>
                            </div>
